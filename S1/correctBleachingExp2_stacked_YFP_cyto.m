function correctBleachingExp2_stacked_YFP_cyto(fitpara,datadir )% fitpara_mRuby
% What it does
...
plot(bleach_YFP); 
 ...
 bleach_mean=nanmedian(bleach_YFP_all,2);

 %fitpara_YFP= fit(timepts',bleach_mean,'exp2');
 %corr=feval(fitpara_YFP,1:length(bleach_mean));
...
for frameNum=1:length(imYFP_raw)
 ...
 im_YFP{frameNum}=imYFP_raw{frameNum}./(bleach_YFP_all(1,frameNum)* bleach_YFP(1, frameNum));
    ...
    tempmRubyforstack=ratio2RGB(im_YFP{frameNum},colorRange2);%Cdc42'
    
    
    tempmRubyforstack(tempmRubyforstack < 0) = 0.001; 
    
    imwrite(tempmRubyforstack,[datadir,filesep, 'YFP_[0,2.5]','.tif'],'WriteMode','append','Compression','none');
   disp(num2str(frameNum)); 
%    
 
end
for i = 1:length(imYFP_raw)
       plot(i, mean(im_YFP{1,i}, 'all')); 
 end 
