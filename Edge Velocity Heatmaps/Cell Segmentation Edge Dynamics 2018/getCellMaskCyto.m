...
function [maskFinal cellCoors] = getCellMaskCyto(image,  minCellSize)
...
imSmooth2=imSmooth1-image_lpf*0.25; % arbitrary scaling factor-- initally 0.25
...
%PixMinMax=double([round(prctile(image(:),1)), round(prctile(image(:),99))]);
%PixMinMax=double([round(prctile(imSmooth2(:),1)), round(prctile(imSmooth2(:),99))]);
...
threshSeg=(x_bgMax+(2)*bgWidth); % for optimal function multiplied by 1.5 or2 because of good fg/bg separationthreshold level based on the background peak
