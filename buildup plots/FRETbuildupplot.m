%% code to compile multiple buildup plots from FRETbuilup.m script. 
% choose a specific edge depth and get mean, 95% conf. interval, etc. 
% the loaded cell arrays have 6 edge depths, which are 3,6,10,15,20,25 px
% at 0.325 um/px (20x no bin, 40x 2x2 bin, etc). 
% this corresponds to (0.975, 1.95, 3.25, 4.875, 6.5, 8.125 um)

clear; 
clc; 

depth_FRET = 2; % here 2 = 6 px, etc
depth_myosin= 5; 
...
% adjust these to size based on total number of events you are compiling. 
% each row is an individual event, columns denote number of frames included
% in the analysis
...
for i=1:4
    ...
    for w=1:size(FRET_temp{2,depth_FRET},2)
        FRET_avg(1,w) =mean(FRET_temp{2,depth_FRET}(:,w));     
    end 

    FRET(i,:) = FRET_avg; 
    
    for w=1:size(myosin_temp{2,depth_myosin},2)
        myosin_avg(1,w) =mean(myosin_temp{2,depth_myosin}(:,w));     
    end 
...
% 
 ...
 for k = 1:46 %  adjust here to length of yourt x axis 
